...
from Other_Functions import Func_Cost_sequence,Func_Cost_sequence_simple, Find_Section_now_num, display_order_list_simple, display_order_list,Check_jam
...
class Simulation:
    ...
    # 【取消过滤的】订单派发算法
    def Func_Assign_Order_simple(self, time):
        order_now = 'error'
        order = Func_Cost_sequence_simple(self.order_notstart, self.section_list, self.order_notstart, self.order_before_section,self.weight)

        # work_step[i][j]：[i]对应第i步；[j=0]对应section名，[j=1]对应工序用时
        work_step = order.work_schedule
        if (int(work_step[0][0]) >= 0):  # 如果第0步不为mainstream，则不堵
            all_order_num = len(self.section_list[int(work_step[0][0])].waiting_order_list) + len(
                self.section_list[int(work_step[0][0])].process_order_list) + len(
                self.section_list[int(work_step[0][0])].finish_order_list)
            if (all_order_num < 6):  # 找到第一步不是mainstream的判断，如果第i步的等待数量小于6，则可以被派发
                order_now = order
            ...
            if (len(self.section_list[int(work_step[0][0])].waiting_order_list) + len(self.section_list[int(
                    work_step[0][0])].finish_order_list) == 0):  # 如果第0步为mainstream，并且第0步等待数量为0，
                if (int(work_step[1][0]) >= 0):  # 如果第1步不为mainstream，则不堵
                    all_order_num = len(self.section_list[int(work_step[1][0])].waiting_order_list) + len(
                        self.section_list[int(work_step[1][0])].process_order_list) + len(
                        self.section_list[int(work_step[1][0])].finish_order_list)
                    if (all_order_num < 6):  # 找到第一步不是mainstream的判断，如果第i步的等待数量小于6，则可以被派发
                        order_now = order
                else:  # 如果第1步为mainstream，则继续判断第1步等待数量
                    if (len(self.section_list[int(work_step[1][0])].waiting_order_list) + len(self.section_list[int(
        ...
        if order_now == 'error':
            if simulation_config['test_version'] ==1:
                print("section已满，本轮无订单被派发")
            return 0
        else:
            # 3\赋予section_now为order_now第一个不为负的section，得到第一个不为负section的编号和当前order所处的工序
    ...
    # 配合GA的订单派发算法
    def Func_Assign_Order_GA(self, time,order_num,type):
        order_now='error'
        if type=="GA":
            for i in range(0,len(self.order_notstart)):
                if self.order_notstart[i].num == order_num:
                    order = self.order_notstart[i]
                    break
                else:
                    continue

        elif type=="rules":
            order = Func_Cost_sequence_simple(self.order_notstart, self.section_list, self.order_notstart,
    ...
    def run(self):
            ...
            if ((t + 1) % self.pace == 0):
                if (len(self.order_notstart) != 0):
                    if (self.type == 'origin'):
                        ...
                        a=self.Func_Assign_Order_GA(time=t,order_num=self.order_list_GA[order_count_GA],type="GA")
                        if a!=0:
                            order_count_GA=order_count_GA+1
                    elif(self.type=='rules_simple'):
                        self.Func_Assign_Order_GA(time=t,order_num=[],type="rules")
        ...
        # 展示数据（总结）
        if (self.op_method=='surrogate') and (self.type=='new'):
...
if __name__ == "__main__":
    ...
    # surrogate_weight = [0.91147,0.98662,0.04665]
    # surrogate_weight=[0.60919189453125,0.095367431640625,0.16668701171875]
    surrogate_weight=[0.64708,0.85714,0]

    # surrogate_weight=[0.44949341, 0.09561157, 0.16668701]
