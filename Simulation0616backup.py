...
class Simulation:
    ...
    def cost_cal_rule(self):
        ...
        #     return line[0]
            ...
            # print(f'N_array.shape:{N_array.shape}')
            T_array = np.dot(self.order_array_weight_all_list[1], self.section_busyness_time_array)  # 订单次序权重 点乘 工站实时工作状态
            # print(f'T_array.shape:{T_array.shape}')
            owntime_array = np.sum(self.order_array_weight_all_list[2]*self.order_array,axis=1).reshape(self.num_order,1)
        ...
        for order in self.order_notstart:
            ...
            for i in range(len(order.work_schedule)):
                if (int(order.work_schedule[i][0]) <0):  # 如果第i步为mainstream，跳过这一步判断下一步是否还为mainstream
                    ...
                    section_waiting_num = len(self.section_list[int(order.work_schedule[i][0])].waiting_order_list) + len(self.section_list[int(order.work_schedule[i][0])].process_order_list) + len(self.section_list[int(order.work_schedule[i][0])].finish_order_list)
                    break
