...
class GaSolver(ea.Problem):
    ...
    def obj_for_parallel(self,x_id,x):
        ...
        results_sim = self.simulation_1.run(rule=xx)  # 运行仿真
...
def trans_x(xx):
    ...
    for i in xx:
        ...
        if(count==3):
            ...
            count=0
            list_inner=[]
    # print(len(list_outer),len(list_outer[0]))
    return list_outer
...
def example_GA(simulation_config):
    ...
    for seed in seed_list:
        ...
        # 评估最优解并输出图像
        test_solution = Simulation(simulation_config)
        answer_list=trans_x(res['Vars'][0])
        print(answer_list)
        print(len(answer_list),len(answer_list[0]))
        results = test_solution.run(rule=answer_list)  # 运行仿真
...
if __name__ == '__main__':
    ...
    print('当前min的目标是:',simulation_config['type'],simulation_config['rule'])
